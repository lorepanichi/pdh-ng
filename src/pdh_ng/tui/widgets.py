...
class StatusBar(Horizontal):
    """Docked horizontal bar with filter cycle buttons and a status/count label."""

    class ScopeChanged(Message):
        """Posted when the scope filter is cycled."""

        def __init__(self, inc_scope: IncScope) -> None:
            """Args:
            inc_scope: The new scope value.
            """
            super().__init__()
            self.inc_scope = inc_scope

    class StatusChanged(Message):
        """Posted when the status filter is cycled."""

        def __init__(self, inc_status: IncStatus) -> None:
            """Args:
            inc_status: The new status filter value.
            """
            super().__init__()
            self.inc_status = inc_status

    class UrgencyChanged(Message):
        """Posted when the urgency filter is cycled."""

        def __init__(self, inc_urgency: IncUrgency) -> None:
            """Args:
            inc_urgency: The new urgency filter value.
            """
            super().__init__()
            self.inc_urgency = inc_urgency

    class RefreshTimeChanged(Message):
        """Posted when the auto-refresh interval is cycled."""

        def __init__(self, refresh_time: RefreshTime) -> None:
            """Args:
            refresh_time: The new refresh interval value.
            """
            super().__init__()
            self.refresh_time = refresh_time

    class AutoAckChanged(Message):
        """Posted when the auto-ack toggle is flipped."""

        def __init__(self, auto_ack: bool) -> None:
            """Args:
            auto_ack: The new auto-ack state.
            """
            super().__init__()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialise the status bar with the current filter and refresh state.

        Args:
            inc_scope: Initial scope filter.
            inc_status: Initial status filter.
            inc_urgency: Initial urgency filter.
            auto_ack: Initial auto-ack state.
            refresh_time: Initial auto-refresh interval.
            **kwargs: Passed through to ``Horizontal.__init__``.
        """
    ...
    def compose(self) -> ComposeResult:
        """Build the status bar layout with filter buttons and a status label."""
    ...
    def set_status(self, inc_count: int, title_filter: str = "") -> None:
        """Update the status label with the incident count and active title filter.

        Args:
            inc_count: Number of incidents currently displayed.
            title_filter: Active title filter string, or empty if none.
        """
    ...
    def set_loading(self) -> None:
        """Append a spinning indicator to the status label while a fetch is in progress."""
    ...
    def set_error(self, message: str) -> None:
        ...
        Args:
            message: Error text to show in bold red.
        """
        self.query_one("#status-label", Label).update(f"   [bold red]{message}[/bold red]")

    def cycle_scope(self) -> None:
        """Advance the scope filter to the next value and post ``ScopeChanged``."""
    ...
    def cycle_status(self) -> None:
        """Advance the status filter to the next value and post ``StatusChanged``."""
    ...
    def cycle_urgency(self) -> None:
        """Advance the urgency filter to the next value and post ``UrgencyChanged``."""
    ...
    def cycle_refresh(self) -> None:
        """Advance the refresh interval to the next value and post ``RefreshTimeChanged``."""
    ...
    def toggle_auto_ack(self) -> None:
        """Toggle auto-ack on/off and post ``AutoAckChanged``."""
...
class ColumnSelectorScreen(ModalScreen):
    ...
    def __init__(self, visible_columns: list[str]) -> None:
        """Args:
        visible_columns: Currently visible columns, used to pre-tick the list.
        """
        super().__init__()
        self._visible_columns = set(visible_columns)

    def compose(self) -> ComposeResult:
        """Build the column selector dialog with a labelled selection list."""
    ...
    def action_confirm(self) -> None:
        """Dismiss with the ordered list of selected columns, or ``None`` if none selected."""
...
class SnoozeDialog(ModalScreen):
    """Modal dialog for selecting a snooze duration (1 h / 4 h / 8 h)."""

    BINDINGS = [("escape", "cancel", "Cancel")]

    def compose(self) -> ComposeResult:
        """Build the snooze dialog with duration buttons."""
